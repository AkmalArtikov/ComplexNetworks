...
def generate_report(dimension, size, threshold, file_name, start_time, distribution, power_law, mode, alpha):
...
def run_generating(dimension, size, threshold, alpha, mode, visualise):
    ...
    graph, coords, weights = generate.get_graph(dimension=dimension, size=size,
                                                threshold=threshold, alpha=alpha, mode=mode)
    ...
    # degree_distribution = analyze.get_degree_distribution(graph)
    ...
    generate_report(dimension=dimension, size=size, threshold=threshold, file_name=file_name, start_time=start_time,
                distribution=graph.degree_distribution(bin_width=1), power_law=power_law, mode=mode, alpha=alpha)

for i in range(0, 10):
    run_generating(dimension=3, size=1000, threshold=2.0, alpha=4.0, mode=1.0, visualise=False)
